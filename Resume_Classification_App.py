...
classification_model = pickle.load(open(r'C:\Users\Navya\Documents\Anaconda_Python\Resume_Classification\XGBmodel.pkl', 'rb'))
features = pickle.load(open(r'C:\Users\Navya\Documents\Anaconda_Python\Resume_Classification\feature.pkl', 'rb'))
...
def main():
    ...
    if submit:
        ...
        if  len(uploaded_files)>0:
            ...
            resume_data['Category'] = resume_data['Category'].map({0: 'React JS Developer', 1 : 'Workday', 2:'Peoplesoft', 3:'SQL Developer'})

    
            st.subheader("Resume Category:")
            for index, row in resume_data.iterrows():  
                st.write(row['File_Name'] ,' : ', row['Category'] )
            st.write('Note : This application classifies Resumes only in Categories : PeopleSoft, React JS Developer, SQL Developer and Workday' ) 
